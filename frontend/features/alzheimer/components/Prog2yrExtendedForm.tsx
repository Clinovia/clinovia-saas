...
export default function Prog2yrExtendedForm({ onSubmit, loading = false }: Props) {
  const [formData, setFormData] = useState<AlzheimerPrognosis2yrExtendedInput>({
    ...
    APOE4_count: 1,
    GDTOTAL: 3,
    ABETA: 900, // example normal-ish value
    TAU: 250,   // example
    PTAU: 22,   // example
  });

  const handleChange = (
    key: keyof AlzheimerPrognosis2yrExtendedInput,
    value: any
  ) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {/* ABETA */}
      <div>
        <label htmlFor="ABETA" className="block font-medium">
          CSF Aβ (ABETA): {formData.ABETA}
        </label>
        <input
          id="ABETA"
          type="range"
          ...
          onChange={(e) => handleChange("ABETA", Number(e.target.value))}
          className="w-full"
        />
      </div>
      ...
      {/* TAU */}
      <div>
        <label htmlFor="TAU" className="block font-medium">
          CSF Tau (TAU): {formData.TAU}
        </label>
        <input
          id="TAU"
          type="range"
          ...
          onChange={(e) => handleChange("TAU", Number(e.target.value))}
          className="w-full"
        />
      </div>
      ...
      {/* PTAU */}
      <div>
        <label htmlFor="PTAU" className="block font-medium">
          CSF p-Tau (PTAU): {formData.PTAU}
        </label>
        <input
          id="PTAU"
          type="range"
          ...
          onChange={(e) => handleChange("PTAU", Number(e.target.value))}
          className="w-full"
        />
      </div>
